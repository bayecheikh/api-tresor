...
class AnalyseGenreController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (!$request->user()->hasRole('expert-sectoriel')) {
            $analyses = AnalyseGenre::paginate(10);  
        }
        else{
            $secteur = User::find($request->user()->id)->secteur[0]->libelle;
            $analyses = AnalyseGenre::where('libelle_secteur', 'like', '%'.$secteur.'%')->paginate(10);
        }
    ...
    {
        if (!$request->user()->hasRole('expert-sectoriel')) {
            $analyses = AnalyseGenre::where('id', 'like', '%'.$term.'%')->orWhere('reference_projet', 'like', '%'.$term.'%')->orWhere('nom_beneficiaire', 'like', '%'.$term.'%')->orWhere('prenom_beneficiaire', 'like', '%'.$term.'%')->orWhere('region', 'like', '%'.$term.'%')->orWhere('departement', 'like', '%'.$term.'%')->orWhere('commune', 'like', '%'.$term.'%')->paginate(10);  
        }
        else{
            $secteur = User::find($request->user()->id)->secteur[0]->libelle;
            $analyses = AnalyseGenre::where('libelle_secteur', 'like', '%'.$secteur.'%')->where('id', 'like', '%'.$term.'%')->orWhere('reference_projet', 'like', '%'.$term.'%')->orWhere('nom_beneficiaire', 'like', '%'.$term.'%')->orWhere('prenom_beneficiaire', 'like', '%'.$term.'%')->orWhere('region', 'like', '%'.$term.'%')->orWhere('departement', 'like', '%'.$term.'%')->orWhere('commune', 'like', '%'.$term.'%')->paginate(10);
        }      
    ...
    public function store(Request $request)
    {
        ...
        $analyse = AnalyseGenre::create($input);       
   ...
   /*          return $this->sendError('Product not found.'); */
        ...
        return response()
            ->json(["success" => true, "message" => "Analyse trouvé avec succès.", "data" => $analyse]);
    }
    /**
